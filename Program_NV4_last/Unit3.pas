...
procedure OrderDelete;       // удаление заявки если она не исполнена
procedure SystemLine;
procedure Op113;
procedure Op214;
procedure SistemW0VN;        // Система W0вн
procedure SistemW0VR;        // Система W0вр
procedure SistemRevolution;  // Система поиска точки переворота
procedure SistemUTurn;       // Схема разворота
procedure GoToStar;          // Система принятия решений
procedure SistemFPSP;        // Схемма определения fp
...
Var w,f3,f4,f5,fp,fo2,sp,tpt,tfp,f7,rn: Integer;
    ...
    mnovn,mnovr,Nk,um1v,um1n,um2v,um2n,tout,frv,frn,mp,mfv,mfn,Xxx: Integer; // числа задаваемые вручную
    ...
    case vf[fp] of
     7,9: begin
       if (rn=0) and (kop=0) and (fp+1<=fk) and (sk[fp+1]=2) and (ks[fp+1,1]=ks[fp,sk[fp]-1]+1) and (vf[fp+1] in [8,10]) then begin
         if (fp-1=0) or (vf[fp-1] in [1,2,3,4,8,10,13,14,98,99]) or ((sk[fp]<3) and (op[fp]=3)) then begin    ///  Схема Перкон
       ...
       case vf[f] of
        1,2,3,7,8,13,14: begin
          ...
          if ks[fo2,1]>1 then begin
            ...
            if (n=0) and (ks[fo2,1]<2) and (Tsm<=1.5) and (vf[f] in [1,3,7,8,13]) and (t[tsk[f,s]]-t[tsl[f,s]]+1>=0.5*(t[tsk[f,s]]-t[tsn[f,s]]+1))
               ...
               or ((max[tsn[fo2,1]]-min[tsp[fo2,1]])/(min[tsp[fo2,1]]-min[tsp[f,s]])>1.9)) then n:=n+3;  // если ks(f,1)=2, то идем в цикл ниже и повторяем условие
        ...
        4,10: begin
          if ( (1+t[tsk[fo2,sk[fo2]]]-t[tsn[fo2,sk[fo2]]]<>2) or ((t[i]=t[tsk[fo2,sk[fo2]]]) and ((max[i-1]<=max[i]) or (min[i-1]>=min[i]))) ) then begin
            ...
            /// if min[tsp[fo2,1]]-cop<[желаяема прибыль] then n:=0;
            if n>2 then begin
              if (op[fo2]<>3) and (op[fo2]<>100) then op[fo2]:=3;
        ...
        1,2,4,9,10,13,14: begin
          ...
          if ks[fo2,1]>1 then begin
            ...
            if (n=0) and (ks[fo2,1]<2) and (Tsm<=1.5) and (vf[f] in [2,4,9,10,14]) and (t[tsk[f,s]]-t[tsl[f,s]]+1>=0.5*(t[tsk[f,s]]-t[tsn[f,s]]+1))
               ...
               or ((max[tsp[fo2,1]]-min[tsn[fo2,1]])/(max[tsp[f,s]]-max[tsp[fo2,1]])>1.9)) then n:=n+3;  // если ks(f,1)=2, то идем в цикл ниже и повторяем условие
        ...
        3,8: begin
          if ( (1+t[tsk[fo2,sk[fo2]]]-t[tsn[fo2,sk[fo2]]]<>2) or ((t[i]=t[tsk[fo2,sk[fo2]]]) and ((max[i-1]<=max[i]) or (min[i-1]>=min[i]))) ) then begin
           ...
           // if (T4>=min[tsk[fo2,1]]-min[tsn[fo2,1]]) and (Tsm>1.18) then n:=0;
            ...
            /// if cop-max[tsp[fo2,1]]<[желаяема прибыль] then n:=0;
            if n>2 then begin
              if (op[fo2]<>3) and (op[fo2]<>100) then op[fo2]:=3;
...
begin
 ...
 if ((f5=0) or (vf[f5] in [1,2,3,4,8,10,13,14,98,99])) and (kop>0) then begin  //схема переноса на fp-1
 ...
 if  pkl=true then begin       //pkl=false and (stat=true)
   ...
   if step1>0 then begin
     ...
     if not (ordNo='') or (ordNo='0') then begin
       ...
       else if step1=2 then begin BB:=1; Form1.Label24.Caption:='Тревога, от АД не получен результат за время step1, '+TimeToStr(ADt);
         step1:=step1-1; Exit;
     ...
     if (stus='W') or (stus='M') then begin
       ...
       else if step3=2 then begin Form1.Label24.Caption:='Тревога, от АД не получен результат за время step3, '+TimeToStr(ADt); step3:=0; Exit;
 ...
 if w=0 then begin
   if ((tpvn>0) and (vf[f3]=7)) or ((tpvr>0) and (vf[f4]=9)) then begin
     ...
     if not (ordNo='') or (ordNo='0') then begin
       ...
       if stus='M' then begin
        ...
        // добавить данные в БД
           ...
           if step6=0 then begin AD.DropOrder(StrToInt(ordNo),'BS',Tread,AccTr,placeC,pcode,tout); step6:=stepN;
             ...
             else if step6=2 then begin BB:=1; Form1.Label24.Caption:='Тревога, от АД не получен результат за время step6, '+TimeToStr(ADt);
               step6:=step6-1; Exit;
             end else step6:=step6-1;
           end;
       ...
       else if step2=2 then begin BB:=1; Form1.Label24.Caption:='Тревога, от АД не получен результат за время step2, '+TimeToStr(ADt);
         step2:=step2-1; Exit;
       end else step2:=step2-1;
     end;
   end;
 end else begin  //если w>0
   ...
   if nap=true then begin
     ...
     //if step3=0 then запись данных в БД
...
begin
 ...
 if vf[f] in [3,8] then begin                                                  // 3,8
   ly[f]:=max[tsk[f,s-1]]-(cp[f,1]-cp[f,s])*(1+t[i]-t[tsk[f,s-1]])/(ysk-y1+1); // Канал Cp tsk s-1
   if s>2 then begin
     ...
     Tx:=max[tsk[f,s-2]]-(cp[f,1]-cp[f,s])*(1+t[i]-t[tsk[f,s-2]])/(ysk-y1+1);    // Канал Cp tsk s-2
   ...
   if (f+1<=fk) and (sk[f+1]=1) and (vf[f+1] in [1,13]) and (1+t[tsk[fo2,1]]-t[tsn[fo2,1]]>3) and (1+t[tsk[fo2,1]]-t[tsn[fo2,1]]>0.111*(1+t[tsk[f,s]]-t[tsn[f,s]]))
      and (tsk[f,s]<>tss[f,s]) then ly[f]:=max[tsn[f,1]]-(max[tsn[f,1]]-max[tss[f,s]])*(1+t[i]-t[tsn[f,1]])/(1+t[tss[f,s]]-t[tsn[f,1]]) // Канал 13/14
   else begin
     Tx:=max[tsk[f,s-1]]-(min[tsp[f,s-1]]-min[tsl[f,s]])*(1+t[i]-t[tsk[f,s-1]])/(1+t[tsl[f,s]]-t[tsp[f,s-1]]);  // Канал tsp(sk)
     ...
     and (1+t[tsk[f,s-1]]-t[tsl[f,s-1]]<=0.33*(1+t[tsk[f,s-1]]-t[tsn[f,s-1]])) then ly[f]:=max[tsp[fo2,1]]-1
       ...
       else if (s=2) and (max[tsn[f,1]]-max[tsk[f,1]]<=0.12*(max[tsn[f,1]]-min[tsp[f,1]])) then begin    // Канал tsn
   ...
   ly[f]:=min[tsk[f,s-1]]+(cp[f,s]-cp[f,1])*(1+t[i]-t[tsk[f,s-1]])/(ysk-y1+1);  // Канал Cp tsk s-1
   if s>2 then begin
     ...
     Tx:=min[tsk[f,s-2]]+(cp[f,s]-cp[f,1])*(1+t[i]-t[tsk[f,s-2]])/(ysk-y1+1);          // Канал Cp tsk s-2
   ...
   if (f+1<=fk) and (sk[f+1]=1) and (vf[f+1] in [2,14]) and (1+t[tsk[fo2,1]]-t[tsn[fo2,1]]>3) and (1+t[tsk[fo2,1]]-t[tsn[fo2,1]]>0.111*(1+t[tsk[f,s]]-t[tsn[f,s]]))
      and (tsk[f,s]<>tss[f,s]) then ly[f]:=min[tsn[f,1]]+(min[tss[f,s]]-min[tsn[f,1]])*(1+t[i]-t[tsn[f,1]])/(1+t[tss[f,s]]-t[tsn[f,1]])  // Канал 13/14
   else begin
     Tx:=min[tsk[f,s-1]]+(max[tsl[f,s]]-max[tsp[f,s-1]])*(1+t[i]-t[tsk[f,s-1]])/(1+t[tsl[f,s]]-(t[tsp[f,s-1]])*1.00001);      // Канал tsp(sk)
     ...
     and (1+t[tsk[f,s-1]]-t[tsl[f,s-1]]<=0.33*(1+t[tsk[f,s-1]]-t[tsn[f,s-1]])) then ly[f]:=min[tsp[fo2,1]]+1
       ...
       if N>=8 then ly[f]:=1111111
       else if (s=2) and (min[tsk[f,1]]-min[tsn[f,1]]<=0.12*(max[tsp[f,1]]-min[tsn[f,1]])) then begin    // Канал tsn
